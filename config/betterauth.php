...
return [
    ...
    |
    | - "hybrid": Tokens + Sessions combined. Bearer token takes priority; falls
    |             back to session auth when no token is present.
    |             Best for: Apps serving both SPA/mobile clients and web browsers.
    |             Guard: betterauth.hybrid
    |
    */
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | When enabled (session mode only), device type and device name are
    | detected from the User-Agent and stored alongside each session record.
    | This enables multi-device session management in your UI.
    |
    | Note: requires the 'device_type' and 'device_name' columns to exist on
    | the sessions table (generated by betterauth:install).
    |
    */
    'device_tracking' => [
        'enabled' => env('BETTER_AUTH_DEVICE_TRACKING', false),
    ],
    ...
    */
    ...
    |
    | @todo Coming in future release: audit logging (log_events) and suspicious
    |       activity detection (detect_suspicious, device_tracking) are not yet
    |       implemented in the Laravel package. These options are reserved for
    |       future use and currently have no effect.
    |
    */
    // 'security' => [
    //     'log_events' => env('BETTER_AUTH_LOG_SECURITY_EVENTS', true),       // @todo Coming in future release
    //     'detect_suspicious' => env('BETTER_AUTH_DETECT_SUSPICIOUS', true),   // @todo Coming in future release
    //     'device_tracking' => env('BETTER_AUTH_DEVICE_TRACKING', true),       // @todo Coming in future release
    // ],
];
